...
public class KafkaConsumer<TKey, TValue>(ConsumerConfig config, IServiceScopeFactory serviceScopeFactory) : IKafkaConsumer<TKey, TValue> where TValue : class
{
    ...
    private static async void OauthTokenRefreshCallback(IClient client, string config)
    {
        try
        {
            ...
            Log.Logger.Debug("ISB Interface Kafka Consumer getting token {0} {1}", tokenEndpoint, clientId);
